...
## run 02-simulations-sec4.R first to get the necessary functions
require(qrng)
require(foreach)
require(doParallel)
require(doSNOW)
...
for (s in 1:3){
  ...
  assign(paste0("res", s, j, "_100"), read.csv(paste0("scen", s, j, "_log_c_100.csv")))
  assign(paste0("res", s, j, "_140"), read.csv(paste0("scen", s, j, "_log_c_140.csv")))
  
  oc.temp <- lin_app(get(paste0("res", s,j,"_100")), get(paste0("res", s,j,"_140")), 
                         100, 140, 80, 160, 0.97)
...
for (s in 1:3){
  ...
  for (j in 1:4){
    ...
    res.lin <- c(res.lin, lin_app(get(paste0("res", s,j,"_100")), get(paste0("res", s,j,"_140")), 
                                          100, 140, 80, 160, gam)[M-79, 2])
  }
}
