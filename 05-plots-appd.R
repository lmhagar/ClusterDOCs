...
for (s in 1:3){
  ...
  for (j in 1:4){
    ...
    ## load in .csv files of posterior probabilities corresponding to c = 25 and c = 45
    assign(paste0("res", s, j, "_25"), read.csv(paste0("scen", s, j, "_log_c_25.csv"))[,1])
    assign(paste0("res", s, j, "_45"), read.csv(paste0("scen", s, j, "_log_c_45.csv"))[,1])
    
    samp1 <- get(paste0("res", s, j, "_25"))
    samp2 <- get(paste0("res", s, j, "_45"))
    ...
    boot.temp <- foreach(k=1:MM, .packages=c('rjags', 'coda', 'purrr'), .combine=rbind,
                         .options.snow=opts) %dopar% {
                           ...
                           bootstrap_doc(samp1.temp, samp2.temp, 25, 45, 20, 50,
                                    gam)
                         }
  ...
  for (j in 1:4){
    ...
    assign(paste0("scen", s, j, ".app"), lin_app(get(paste0("res", s,j,"_25")), get(paste0("res", s,j,"_45")), 
                                               25, 45, 20, 50, gam))
  ...
  for (j in 1:4){
    assign(paste0("df", s, j),
           data.frame(c = c(get(paste0("scen", s, j, ".app"))[,1], Ms,
                      ...
                      type = c(rep(1, 31), rep(2, 7), rep(3, 31), rep(4, 31))))
  }
...
for (s in 1:3){
  ...
  pdf(file = paste0("Fig", s, "DOC_App.pdf"),   # The directory you want to save the file in
